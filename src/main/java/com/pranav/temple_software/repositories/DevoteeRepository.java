...
public class DevoteeRepository {
	...
	/**
	 * Finds the most recent details of a devotee from either the Receipts or
	 * DonationReceipts table based on their phone number.
	 *
	...
	public Optional<DevoteeDetails> findLatestDevoteeDetailsByPhone(String phoneNumber) {
		// This SQL query combines results from two tables, orders them by the most recent timestamp,
		// and picks the top one.
		String sql = "SELECT devotee_name, address, rashi, nakshatra FROM (" +
				"  SELECT devotee_name, address, rashi, nakshatra, timestamp FROM Receipts WHERE phone_number = ? " +
				"  UNION ALL " +
				"  SELECT devotee_name, address, rashi, nakshatra, timestamp FROM DonationReceipts WHERE phone_number = ? " +
		...
		try (Connection conn = DatabaseManager.getConnection();
			...
			if (rs.next()) {
				...
				DevoteeDetails details = new DevoteeDetails(name, address, rashi, nakshatra);
